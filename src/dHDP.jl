...
function truncated_gibbs_sampler{T1, T2}(
    ...
    for kk = 1:KK_truncation-1
        gamma1 = 1 + sum(nn[:, kk])
        gamma2 = dhdp.gg + sum(nn[:, kk+1:KK_truncation])
    ...
    for ll = 1:n_groups
        for kk = 1:KK_truncation-1
            gamma1 = dhdp.aa * my_beta[kk] + nn[ll, kk]
            gamma2 = dhdp.aa * (1 - sum(my_beta[1:kk])) + sum(nn[ll, kk+1:KK_truncation])
    ...
    for iteration = 1:n_iterations
        ...
        dhdp.KK = count_active_clusters(nn)
                    ...
                    for kk = 1:KK_truncation
                        if sum(nn[:, kk]) != 0
                            if nn[jj, kk] == 0
                                ...
                                pp_rr = zeros(Float64, nn[jj, kk])
                                for mm = 1:nn[jj, kk]
                                    pp_rr[mm] = log(snumbers[nn[jj, kk], mm]) + mm*log(dhdp.aa * my_beta[kk])
                                end
                                lognormalize!(pp_rr)
